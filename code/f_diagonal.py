...
def time_tab(t_i,t_f,Nstep,Lo_li):
	...
	if Lo_li == 0:
		#t_tab = np.insert(0,1,np.linspace(t_i,t_f, num=Nstep, dtype=float))
		t_tab = np.linspace(t_i,t_f, num=Nstep, dtype=float)
	else:
...
def ExactDiagonalization(PATH_now,L,D): #,Tab_CdC):
	...
	print('t_0',t_i,'t_f',t_f,'n_step',step_num)
	...
	#psit1    = linalg.expm(t_f*A).dot(psi_0)

	#print(np.sum(psit[step_num]-psit1))



	#.............................Diagonalization HAM
	#HAMD   = ff.Ham_Dense_Creation(LL,NN,Dim,DD,Dis_real,BC,Base_Bin,Base_Num,Hop_Bin,LinTab)
